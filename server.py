...
def _kill_all_cloudflared():
    """Disabled to prevent tearing down existing stable tunnels and cache."""
    pass
...
def _start_leadflow_tunnel() -> str:
    ...
    # Check for existing system-wide cloudflared processes running LeadFlow
    import subprocess
    try:
        proc_check = subprocess.run(["pgrep", "-f", "cloudflared tunnel --url http://127.0.0.1:8765"], capture_output=True, text=True)
        if proc_check.stdout.strip():
            url = Path("/tmp/leadflow-tunnel-url.txt").read_text().strip()
            if url.startswith("https://"):
                print(f"[tunnel] Found existing cloudflared process, reusing URL: {url}")
                return url
    except Exception:
        pass
    ...
    if _leadflow_tunnel_proc and _leadflow_tunnel_proc.poll() is None:
        try:
            url = Path("/tmp/leadflow-tunnel-url.txt").read_text().strip()
            if url.startswith("https://"):
                print(f"[tunnel] Reusing existing tunnel proc: {url}")
                return url
        except Exception:
            pass
...
async def lifespan(app: FastAPI):
    global _demo_proc
    # ── Removed blind kill to allow tunnel reuse upon reload ──
    # Only kill tunnels on hard shutdown, NOT on startup.
...
def api_phone_status():
    """Returns who is controlling the Vivo phone and what it's doing."""
    try:
        from ig_phone_status import get_phone_status, migrate as ps_migrate
        ps_migrate()
        return JSONResponse(content=get_phone_status())
    except Exception as e:
        log.error(f"Phone status API error: {e}")
        return JSONResponse(
            status_code=500,
            content={"is_active": False, "error": str(e)}
        )
...
async def whatsapp_webhook(request: Request):
    """Twilio WhatsApp webhook — fires when a prospect replies via WA."""
    try:
        form = await request.form()
        from_wa = form.get("From", "")
        body = form.get("Body", "")

        logging.info(f"[WhatsApp] Reply received from {from_wa}: {body[:50]}...")

        if not from_wa:
            return JSONResponse({"status": "ignored", "reason": "no_from"})

        # Normalize phone: remove 'whatsapp:' prefix
        raw_phone = from_wa.replace("whatsapp:", "")
        search_phone = "%" + raw_phone[-10:] if len(raw_phone) >= 10 else "%" + raw_phone.lstrip("+")

        from database import get_conn
        conn = get_conn()
        try:
            # Find the business match by phone
            cur = conn.cursor()
            cur.execute("SELECT id FROM businesses WHERE phone LIKE ? OR phone = ?", (search_phone, raw_phone))
            biz_row = cur.fetchone()

            if biz_row:
                biz_id = biz_row["id"]
                # Mark outreach as replied for this business
                cur.execute("""
                    UPDATE outreach
                    SET replied = 1
                    WHERE business_id = ?
                ...
                if cur.rowcount > 0:
                    logging.info(f"[WhatsApp] Marked outreach replied=1 for business_id={biz_id}")
                conn.commit()
                return JSONResponse({"status": "success"})
            else:
                logging.info(f"[WhatsApp] No business found for phone={raw_phone}")
                return JSONResponse({"status": "ignored", "reason": "no_business_match"})
...
@app.get("/api/firestick-url")
def firestick_url():
    """Return the Firestick dashboard URL read from .firestick_ip file."""
...
def leads_by_filter(filter: str = "total_scraped"):
    """Return up to 200 leads matching a stat-card filter key for the dashboard drill-down modal."""
    conn = get_conn()
    try:
        rows = []
        if filter == "total_scraped":
            # All businesses, newest first
            raw = conn.execute("""
                SELECT b.id, b.name, b.category, b.website, b.status, b.found_at as ts,
                       b.demo_viewed, c.email,
                       NULL as opened, NULL as clicked, NULL as replied
                FROM businesses b
                LEFT JOIN contacts c ON c.business_id = b.id
                ORDER BY b.found_at DESC LIMIT 200
            """).fetchall()
            rows = [dict(r) for r in raw]

        elif filter == "total_sent":
            # Email outreach rows that have been sent
            raw = conn.execute("""
                SELECT b.id, b.name, b.category, b.website, b.status, o.sent_at as ts,
                       b.demo_viewed, c.email,
                       o.opened, o.clicked, o.replied
                FROM outreach o
                JOIN businesses b ON b.id = o.business_id
                LEFT JOIN contacts c ON c.business_id = b.id
        ...
        elif filter == "total_followups":
            # Follow-up rows that have been sent
            raw = conn.execute("""
                SELECT b.id, b.name, b.category, b.website, b.status, f.sent_at as ts,
                       b.demo_viewed, c.email,
                       NULL as opened, NULL as clicked, NULL as replied
                FROM follow_ups f
                JOIN businesses b ON b.id = f.business_id
                LEFT JOIN contacts c ON c.business_id = b.id
                WHERE f.status = 'sent' AND f.channel = 'email'
                ORDER BY f.sent_at DESC LIMIT 200
            """).fetchall()
            rows = [dict(r) for r in raw]

        elif filter == "total_opened":
            raw = conn.execute("""
                SELECT b.id, b.name, b.category, b.website, b.status, o.sent_at as ts,
                       b.demo_viewed, c.email,
                       o.opened, o.clicked, o.replied
            ...
            raw = conn.execute("""
                ...
                FROM businesses b
                LEFT JOIN contacts c ON c.business_id = b.id
                WHERE b.demo_viewed = 1
                ORDER BY b.found_at DESC LIMIT 200
            """).fetchall()
            rows = [dict(r) for r in raw]

        elif filter == "total_replied":
            raw = conn.execute("""
                SELECT b.id, b.name, b.category, b.website, b.status, o.sent_at as ts,
                       b.demo_viewed, c.email,
                       o.opened, o.clicked, o.replied
